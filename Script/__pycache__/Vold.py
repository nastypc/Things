...
try:
    ...
    def create_material_to_breakdown_mapping(mats):
        ...
        # Generate alphabetical labels (A, B, C, D, ..., AA, AB, AC, etc.)
        ...
        try:
            with open(expected_path, 'w', encoding='utf-8') as fh:
                ...
                for pname, pobj in sorted_panels_dict.items():
                    ...
                    try:
                        # Extract SubAssemblies from panel materials
                        subassembly_details = {}
                        
                        mats_list = materials_map.get(pname, [])
                        ...
                        for m in mats_list:
                            ...
                            # Only process materials that belong to a SubAssembly
                            if subassembly_guid and subassembly_name:
                                # Skip beam pockets and rough openings (they're handled separately)
                                if ('beampocket' in subassembly_name.lower() or 
                                    'roughopening' in subassembly_name.lower() or
                                    'rough_opening' in subassembly_name.lower()):
                                    continue
                                
                                # Skip End Stud Assembly
                                if 'end stud assembly' in subassembly_name.lower():
                                    continue
                                
                                # Define allowed FamilyMember IDs for filtering
                                allowed_family_members = [32, 42]  # Only Critical Stud (32) and Ladder (42)
                                
                                # Map FamilyMemberName to FamilyMember ID for filtering
                                family_member_id = None
                                if family_member_name:
                                    # Skip End Stud Assembly explicitly
                                    if 'end stud assembly' in family_member_name.lower():
                                        continue
                                    
                                    # FamilyMember 32 (Critical Stud) mappings
                                    if (family_member_name in ['2S LType', '3S LType', '4S LType', 'Critical Stud', 
                                                             'Flat Stud', 'FlatStud', 'LType', 'LType M', 'Stud', 'Tee', 'U Type'] or
                                        'LType' in family_member_name or family_member_name == 'LType'):
                                        family_member_id = 32
                                    # FamilyMember 42 (Ladder) mappings
                                    elif (family_member_name in ['Lad2Stud', 'Ladder - Flat (Fixed)', 'Ladder - Flat (Odd)'] or
                                          'Lad2Stud' in family_member_name or family_member_name == 'Lad2Stud' or
                                          'Ladder' in family_member_name):
                                        family_member_id = 42
                                    # Other FamilyMember mappings (unchanged)
                                    elif 'BSMT-HDR' in family_member_name or family_member_name == 'BSMT-HDR':
                                        family_member_id = 25
                                    elif 'Sheathing' in family_member_name or family_member_name == 'Sheathing':
                                        family_member_id = 40
                                    elif '49x63-L2' in family_member_name or family_member_name == '49x63-L2':
                                        family_member_id = 25  # 49x63-L2 also has FamilyMember 25
                                
                                # Filter: include materials from LType SubAssemblies OR allowed FamilyMember IDs
                                if subassembly_name == 'LType' or family_member_id in allowed_family_members:
                                    # Initialize SubAssembly if not seen before
                                    if subassembly_guid not in subassembly_details:
                                        subassembly_details[subassembly_guid] = {
                                    ...
                                    # Count materials by label
                                    if label:
                                        if label not in subassembly_details[subassembly_guid]['materials']:
                                            subassembly_details[subassembly_guid]['materials'][label] = 0
                                        subassembly_details[subassembly_guid]['materials'][label] += 1
                            
                        # Also process individual materials that should be grouped as Critical Stud/Ladder components
                        if label and m.get('Type', '') and not m.get('SubAssemblyGuid') and not m.get('SubAssembly'):
                            material_type = m.get('Type', '')
                            # Get the actual FamilyMember ID
                            material_family_member_str = m.get('FamilyMember', '')
                            material_family_member = None
                            try:
                                if material_family_member_str:
                                    material_family_member = int(material_family_member_str)
                            except (ValueError, TypeError):
                                material_family_member = None
                            
                            # Group as Critical Stud (32) or Ladder (42) components
                            if material_family_member in [32, 42]:
                                # Use normalized name based on FamilyMember ID for consistent grouping
                                if material_family_member in [32, 42]:
                                    base_family_name = 'LType' if material_family_member == 32 else 'Ladder'
                                else:
                                    # Fallback to extracting from FamilyMemberName
                                    base_family_name = family_member_name
                                    if ' ' in family_member_name:
                                        parts = family_member_name.split(' ', 1)
                                        if len(parts) == 2 and len(parts[1]) == 1:  # Single character suffix
                                            base_family_name = parts[0]
                                
                                # Check if there's already a SubAssembly with this base name
                                existing_sub_guid = None
                                for sub_guid, sub_info in subassembly_details.items():
                                    if sub_info['name'] == base_family_name and sub_info['family_member'] == material_family_member:
                                        existing_sub_guid = sub_guid
                                        break
                                
                                if existing_sub_guid:
                                    # Add to existing SubAssembly
                                    if label not in subassembly_details[existing_sub_guid]['materials']:
                                        subassembly_details[existing_sub_guid]['materials'][label] = 0
                                    subassembly_details[existing_sub_guid]['materials'][label] += 1
                                else:
                                    # Create new synthetic entry with base name
                                    synthetic_guid = f'synthetic_{base_family_name}'
                                    synthetic_name = base_family_name
                                    synthetic_family_member = material_family_member
                                    
                                    if synthetic_guid not in subassembly_details:
                                        subassembly_details[synthetic_guid] = {
                                            'name': synthetic_name,
                                            'family_member': synthetic_family_member,
                                            'family_member_name': family_member_name,
                                            'materials': {}
                                        }
                                    
                                    # Count materials by label
                                    if label not in subassembly_details[synthetic_guid]['materials']:
                                        subassembly_details[synthetic_guid]['materials'][label] = 0
                                    subassembly_details[synthetic_guid]['materials'][label] += 1
                        ...
                        if subassembly_details:
                            ...
                            for sub_guid, sub_info in subassembly_details.items():
                                ...
                                # Use the actual FamilyMemberName for display
                                family_member_name = sub_info.get('family_member_name', sub_name)
            ...
            with open(dest, 'w', encoding='utf-8') as out:
                ...
                try:
                    ...
                    for m in mats_list:
                        ...
                        # Only process materials that belong to a SubAssembly
                        if subassembly_guid and (subassembly_name or subassembly_guid == '43214eec-9b08-4efa-abbb-901178098d1e'):
                            # Skip beam pockets and rough openings (they're handled separately)
                            if ('beampocket' in subassembly_name.lower() or 
                                'roughopening' in subassembly_name.lower() or
                                'rough_opening' in subassembly_name.lower()):
                            ...
                            # Get the actual FamilyMember ID from the material
                            family_member_id_str = m.get('FamilyMember', '')
                            family_member_id = None
                            try:
                                if family_member_id_str:
                                    family_member_id = int(family_member_id_str)
                            except (ValueError, TypeError):
                                family_member_id = None
                            
                            # Filter: include LType SubAssembly materials or FamilyMember IDs 32 (Critical Stud) and 42 (Ladder), exclude 6 (Loose Members)
                            if subassembly_guid == '43214eec-9b08-4efa-abbb-901178098d1e' or family_member_id in [32, 42]:
                                # Initialize SubAssembly if not seen before
                                if subassembly_guid not in subassembly_details:
                                    subassembly_details[subassembly_guid] = {
                                        'name': subassembly_name or 'LType',
                                        'family_member': family_member_id,
                                        'materials': {}
                                    }
                        ...
                        # Also process individual materials that should be grouped as Critical Stud/Ladder components
                            ...
                            # If FamilyMember ID is not set, try to map from FamilyMemberName
                            if material_family_member is None:
                                family_member_name = m.get('FamilyMemberName', '')
                                if family_member_name:
                                    # FamilyMember 32 (Critical Stud) mappings
                                    if (family_member_name in ['2S LType', '3S LType', '4S LType', 'Critical Stud', 
                                                             'Flat Stud', 'FlatStud', 'LType', 'LType M', 'Stud', 'Tee', 'U Type'] or
                                        'LType' in family_member_name or family_member_name == 'LType'):
                                        material_family_member = 32
                                    # FamilyMember 42 (Ladder) mappings
                                    elif (family_member_name in ['Lad2Stud', 'Ladder - Flat (Fixed)', 'Ladder - Flat (Odd)'] or
                                          'Lad2Stud' in family_member_name or family_member_name == 'Lad2Stud' or
                                          'Ladder' in family_member_name):
                                        material_family_member = 42
                            
                            # Only group as Critical Stud if it's actually FamilyMember 32 (Critical Stud) or 42 (Ladder)
                            if material_family_member in [32, 42]:
                                # Use the actual FamilyMemberName instead of synthetic name
                                actual_family_name = m.get('FamilyMemberName', 'Critical Stud')
                                
                                # Extract base name by removing label suffix (e.g., "LType K" -> "LType")
                                # Use normalized name based on FamilyMember ID for consistent grouping
                                if material_family_member in [32, 42]:
                                    base_family_name = 'LType' if material_family_member == 32 else 'Ladder'
                                else:
                                    base_family_name = actual_family_name
                                    if ' ' in actual_family_name:
                                        parts = actual_family_name.split(' ', 1)
                                        if len(parts) == 2 and len(parts[1]) == 1:  # Single character suffix
                                            base_family_name = parts[0]
                                
                                synthetic_guid = f'synthetic_{base_family_name}'
                                synthetic_name = base_family_name
                                synthetic_family_member = 32
                    ...
                    if subassembly_details:
                        out.write("Subcomponent Details:\n")
                        ...
                        for sub_guid, sub_info in subassembly_details.items():
                            ...
                            # Normalize display name based on FamilyMember ID (same as GUI display)
                            family_member_id = sub_info['family_member']
                            if family_member_id == 32:
                                display_name = sub_name
                            elif family_member_id == 42:
                                display_name = sub_name
                            else:
                                display_name = sub_name
                            ...
                            if materials_dict:
                                ...
                                for mat_label, count in sorted(materials_dict.items()):
                                    ...
                                    out.write(f"    ├── {breakdown_label} ({count})\n")
            ...
            if panel_obj and isinstance(panel_obj, dict):
                ...
                try:
                    ...
                    for m in mats_list:
                        ...
                        # Process materials that belong to a SubAssembly
                        if subassembly_guid and (subassembly_name or subassembly_guid == '43214eec-9b08-4efa-abbb-901178098d1e'):
                            # Skip beam pockets and rough openings (they're handled separately)
                            if ('beampocket' in subassembly_name.lower() or 
                                'roughopening' in subassembly_name.lower() or
                            ...
                            # Filter: include LType SubAssembly materials or FamilyMember 32 (Critical Stud) and 42 (Ladder), exclude 25 and 40
                            if subassembly_guid == '43214eec-9b08-4efa-abbb-901178098d1e' or family_member_id in [32, 42]:
                                # Initialize SubAssembly if not seen before
                                if subassembly_guid not in subassembly_details:
                                    subassembly_details[subassembly_guid] = {
                                        'name': subassembly_name or 'LType',  # Use 'LType' as fallback name
                                        'family_member': family_member_id,
                                        'family_member_name': family_member_name,
                                        'materials': {}
                                    }
                        ...
                        # Also process individual materials that should be grouped as Critical Stud/Ladder components
                        elif label and material_type:
                            # Group Stud and FlatStud materials as Critical Stud components
                            if material_type in ['Stud', 'FlatStud']:
                                # Use the actual FamilyMemberName instead of synthetic name
                                actual_family_name = family_member_name if family_member_name else 'Critical Stud'
                                
                                # Get the FamilyMember ID for this material
                                material_family_member_str = m.get('FamilyMember', '')
                                material_family_member = None
                                try:
                                    if material_family_member_str:
                                ...
                                synthetic_guid = f'synthetic_{base_family_name}'
                                synthetic_name = base_family_name
                                synthetic_family_member = material_family_member if material_family_member else 32
                    ...
                    if subassembly_details:
                        # Filter to only show subassemblies with FamilyMember 32 and 42
                        filtered_subassemblies = {}
                        for sub_guid, sub_info in subassembly_details.items():
                            # Include all subassemblies that have FamilyMember 32 (Critical Stud) or 42 (Ladder)
                            if sub_info['family_member'] in [32, 42]:
                                filtered_subassemblies[sub_guid] = sub_info
                        
                        if filtered_subassemblies:
                            # Add SubAssemblies header with count
                            subassembly_header = tk.Frame(details_scrollable_frame, bg='#32CD32')  # Lime green color
                            subassembly_header.pack(fill='x', padx=8, pady=4)
                            total_filtered = len(filtered_subassemblies)
                            tk.Label(subassembly_header, text='Subcomponent Details',
                            ...
                            # Display each filtered SubAssembly
                            for sub_guid, sub_info in filtered_subassemblies.items():
                                sub_name = sub_info['name']
                                materials_dict = sub_info['materials']
                                ...
                                # Display SubAssembly name directly without type labels
                                add_detail_line(f"• {display_name}", None, bullet=True, raw=True)
                                
                                # Display materials if any
                                if materials_dict:
                                    add_detail_line("   Materials:", None, bullet=True, raw=True)
                                    for mat_label, count in sorted(materials_dict.items()):
                                        # Find the corresponding alphabetical label from the breakdown mapping
                                        breakdown_label = mat_label  # Default to original label
                                        ...
                                        add_detail_line(f"    ├── {breakdown_label} ({count})", None, bullet=True, raw=True)
                except Exception as e:
