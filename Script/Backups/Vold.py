...
def make_gui():
        ...
        try:
            ...
            with open(dest, 'w', encoding='utf-8') as out:
                ...
                try:
                    ...
                    # Use the new parser to read Subcomponent Details from expected.log
                    expected_log_path = os.path.join(HERE, 'LOG', f'{job_val.cget("text")}.log')
                    subassembly_details = parse_subcomponent_details_from_expected_log(expected_log_path, display_name)
                    ...
                    if subassembly_details:
                        print(f"DEBUG: Writing Subcomponent Details section")
                        ...
                        for sub_guid, sub_info in subassembly_details.items():
                            ...
                            if materials_dict:
                                ...
                                out.write('\n')
                        print(f"DEBUG: Finished writing Subcomponent Details")
                    else:
    ...
    def display_panel(name, panel_obj, materials):
        ...
        try:
            ...
            pass
        # Function to toggle subcomponent details visibility
        def toggle_subcomponent_details(button, content_frame):
            try:
                if content_frame.winfo_ismapped():
                    ...
                    for child in button.master.winfo_children():
                        if isinstance(child, tk.Label) and 'Subcomponent Details' in child.cget('text'):
                            child.config(text='Subcomponent Details (Click to expand)')
                            break
                else:
                    ...
                    for child in button.master.winfo_children():
                        if isinstance(child, tk.Label) and 'Subcomponent Details' in child.cget('text'):
                            child.config(text='Subcomponent Details (Click to collapse)')
                            break
            except Exception:
                pass
        ...
        try:
            if panel_obj and isinstance(panel_obj, dict):
                ...
                try:
                    ...
                    for ro in ro_list:
                        for line in ro:
                            add_detail_line(line, None, bullet=True, raw=True, parent_frame=tech_specs_content)
                ...
                try:
                    ...
                    if subassembly_details:
                        # Create a collapsible frame for Subcomponent Details
                        subcomponent_frame = tk.Frame(details_scrollable_frame, bg=DETAILS_BG)
                        subcomponent_frame.pack(fill='x', padx=8, pady=4)
                        
                        # Header with toggle button
                        subassembly_header = tk.Frame(subcomponent_frame, bg='#32CD32')  # Lime green color
                        subassembly_header.pack(fill='x')
                        
                        # Toggle button
                        toggle_btn = tk.Button(subassembly_header, text='▶', bg='#32CD32', fg='white',
                                             font=('Segoe UI', 10, 'bold'), bd=0, padx=2,
                                             command=lambda: toggle_subcomponent_details(toggle_btn, subcomponent_content))
                        toggle_btn.pack(side='left', padx=(6, 2), pady=3)
                        
                        # Header text
                        tk.Label(subassembly_header, text='Subcomponent Details (Click to expand)',
                                bg='#32CD32', fg='white', font=('Segoe UI', 10, 'bold'),
                                anchor='w').pack(side='left', padx=(2, 6), pady=3)
                        
                        # Content frame (initially hidden)
                        subcomponent_content = tk.Frame(subcomponent_frame, bg=DETAILS_BG)
                        # Don't pack it yet - it will be shown when toggled
                        ...
                        for sub_guid, sub_info in subassembly_details.items():
                            ...
                            # Display SubAssembly name directly without type labels
                            add_detail_line(f"• {sub_name}", None, bullet=True, raw=True, parent_frame=subcomponent_content)
                            
                            # Display materials if any
                            if materials_dict:
                                add_detail_line("   Materials:", None, bullet=True, raw=True, parent_frame=subcomponent_content)
                                for mat_label, count in sorted(materials_dict.items()):
                                    # Use the material label directly from the log file
                                    add_detail_line(f"    ├── {mat_label} ({count})", None, bullet=True, raw=True, parent_frame=subcomponent_content)
                except Exception as e:
