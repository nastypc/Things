...
try:
    ...
    def parse_panels(path):
        ...
        for junction in root.findall('.//Junction'):
            ...
            if subassembly_name_el is not None and subassembly_name_el.text:
                ...
                # Store all SubAssemblyName values as junction types
                if subassembly_name:
                    junction_info[subassembly_name] = subassembly_name
        ...
        # Always create diag_report for legacy files to ensure correct diagnostic header
        diag_report = {
            'junction_details': junction_details,
            'junctions_found': len([j for j in root.findall('.//Junction')]),
            'bundles_found': 0,  # Legacy files don't have bundles
            'panels_total': len(panels),  # Use panels list length for now
            'panels_assigned': len(panels),  # Assume all panels are assigned in legacy
            'panels_unassigned': 0,
            'junction_mappings': {},
            'bundle_layer_mappings': {},
            'assignment_details': []
        ...
        try:
            ...
            # diag_report should already be set by legacy junction extraction above
            print(f"DEBUG: diag_report value before writing logs: {diag_report}")
            print(f"DEBUG: diag_report type: {type(diag_report)}")
            if diag_report and 'junction_details' in diag_report:
                print(f"DEBUG: junction_details keys: {list(diag_report['junction_details'].keys())}")
    ...
    def write_expected_and_materials_logs(ehx_path, panels_by_name, materials_map, diag_report=None):
        ...
        ehx_version = "legacy"
        # diag_report may have been set by legacy junction extraction above
        if 'diag_report' not in locals():
            diag_report = None
        try:
            # Try to detect version from the file content
            tree = ET.parse(ehx_path)
            root = tree.getroot()
            
            # Check for EHXVersion element to properly detect V2.0
            ehx_version_element = root.find('EHXVersion')
            if ehx_version_element is not None and ehx_version_element.text:
                version_text = ehx_version_element.text.strip()
                if version_text == "2.0":
                    ehx_version = "v2.0"
                else:
                    ehx_version = f"v{version_text}"
            else:
                # No EHXVersion element found, check filename for clues
                fname_lower = os.path.basename(ehx_path).lower()
                if 'mpo' in fname_lower or 'v2' in fname_lower:
                    ehx_version = "v2.0"
                # Legacy format doesn't have EHXVersion element
            
            # Only call diagnose_v2_bundle_assignment for V2.0 files
            if ehx_version == "v2.0":
                diag_report = diagnose_v2_bundle_assignment(root, ehx_version, panels_by_name)
            # For legacy files, diag_report is already created above in the junction extraction section
        except Exception as e:
            print(f"Diagnostic setup error: {e}")
            
        if diag_report and unassigned_panels:
            version_display = "V2.0" if ehx_version == "v2.0" else "LEGACY"
            print(f"\n=== {version_display} DIAGNOSTIC REPORT ===")
        ...
        try:
            with open(expected_path, 'w', encoding='utf-8') as fh:
                ...
                # Add diagnostic info for files with junction details
                print(f"DEBUG: About to check diag_report: {diag_report}")
                print(f"DEBUG: diag_report type: {type(diag_report)}")
                if diag_report:
                    version_display = "V2.0" if ehx_version == "v2.0" else "LEGACY"
                    print(f"DEBUG: Writing diagnostic header with ehx_version='{ehx_version}', version_display='{version_display}'")
                    fh.write(f"\n=== {version_display} DIAGNOSTIC INFO ===\n")
                    ...
                    print("DEBUG: diag_report is None or empty, skipping diagnostic header")
                ...
                for pname, pobj in sorted_panels_dict.items():
                    ...
                    # Add Junction Details after Rough Openings
                    try:
                        panel_junction_details = None
                        if diag_report and 'junction_details' in diag_report:
                            # Try to find junction details using panel name or display name
                            panel_junction_details = diag_report['junction_details'].get(pname)
                            if not panel_junction_details:
                                display_name = pobj.get('DisplayLabel', pname)
                                panel_junction_details = diag_report['junction_details'].get(display_name)
                        
                        print(f"DEBUG: For panel {pname}, panel_junction_details: {panel_junction_details}")
                        if panel_junction_details:
                            print(f"DEBUG: Writing junction details for panel {pname}")
                            fh.write("Junction Details:\n")
                            # Display all junction types found
                            for junction_type in panel_junction_details:
                                fh.write(f"• {junction_type} (1)\n")
                            fh.write('\n')
                        else:
                            print(f"DEBUG: No junction details found for panel {pname}")
                    except Exception as e:
...
def make_gui():
    ...
    def export_current_panel():
        try:
            ...
            with open(dest, 'w', encoding='utf-8') as out:
                ...
                # Add Junction Details after Rough Openings
                try:
                    # Get junction details for this panel by parsing the EHX file
                    panel_junction_details = None
                    ...
                    if current_ehx_path and os.path.exists(current_ehx_path):
                        try:
                            # Parse the EHX file to find junctions
                            tree = ET.parse(current_ehx_path)
                            root_xml = tree.getroot()
                            
                            # Look for junctions related to this panel
                            junction_details = {}
                            for junction in root_xml.findall('.//Junction'):
                                panel_id_el = junction.find('PanelID')
                                label_el = junction.find('Label')
                                panel_id = panel_id_el.text.strip() if panel_id_el is not None and panel_id_el.text else None
                                label = label_el.text.strip() if label_el is not None and label_el.text else None
                                
                                if panel_id == sel_name or label == sel_name or panel_id == panel_obj.get('DisplayLabel', sel_name):
                                    # Extract junction details from SubAssemblyName
                                    subassembly_name_el = junction.find('SubAssemblyName')
                                    if subassembly_name_el is not None and subassembly_name_el.text:
                                        subassembly_name = subassembly_name_el.text.strip()
                                        ...
                                        # Parse SubAssemblyName to extract junction details
                                        if subassembly_name == 'LType':
                                            junction_details['LType'] = 'LType'
                                        elif subassembly_name.startswith('Ladder'):
                                            junction_details['Ladder'] = subassembly_name
                                        elif subassembly_name == 'Subcomponent':
                                            junction_details['Subcomponent'] = 'Subcomponent'
                            
                            if junction_details:
                                panel_junction_details = junction_details
                        except Exception as e:
                            print(f"DEBUG: Exception parsing junctions for export: {e}")
                    
                    if panel_junction_details:
                        out.write("Junction Details:\n")
                        if 'LType' in panel_junction_details:
                            out.write(f"• LType (1)\n")
                        if 'Ladder' in panel_junction_details:
                            out.write(f"• Ladder (1)\n")
                        if 'Subcomponent' in panel_junction_details:
                            out.write(f"• Subcomponent (1)\n")
                        out.write("\n")
                except Exception as e:
                    print(f"DEBUG: Exception in junction details for export: {e}")
    ...
    def display_panel(name, panel_obj, materials):
        ...
        try:
            if panel_obj and isinstance(panel_obj, dict):
                ...
                # Add Junction Details section after Rough Openings
                try:
                    # Create junction details for this panel
                    panel_junction_details = None
                    ...
                    if current_ehx_path and os.path.exists(current_ehx_path):
                        try:
                            ...
                            for junction in root_xml.findall('.//Junction'):
                                ...
                                if panel_id == name or label == name or panel_id == panel_obj.get('DisplayLabel', name):
                                    # Extract junction details from SubAssemblyName
                                    subassembly_name_el = junction.find('SubAssemblyName')
                                    if subassembly_name_el is not None and subassembly_name_el.text:
                                        subassembly_name = subassembly_name_el.text.strip()
                                        ...
                                        if subassembly_name == 'LType':
                                            junction_counts['LType'] = junction_counts.get('LType', 0) + 1
                                        elif subassembly_name.startswith('Ladder'):
                                            junction_type = 'Ladder'
                                            junction_counts[junction_type] = junction_counts.get(junction_type, 0) + 1
                                        elif subassembly_name == 'Subcomponent':
                                            junction_counts['Subcomponent'] = junction_counts.get('Subcomponent', 0) + 1
                            
                            if junction_counts:
                                panel_junction_details = junction_counts
                        except Exception as e:
                            print(f"DEBUG: Exception parsing junctions for GUI: {e}")
                    
                    if panel_junction_details:
                        # Add Junction Details header
                        junction_header = tk.Frame(details_scrollable_frame, bg=WARNING_ORANGE)
                        ...
                        if 'LType' in panel_junction_details:
                            add_detail_line(f"LType ({panel_junction_details['LType']})", None, bullet=True, raw=True)
                        if 'Ladder' in panel_junction_details:
                            add_detail_line(f"Ladder ({panel_junction_details['Ladder']})", None, bullet=True, raw=True)
                        if 'Subcomponent' in panel_junction_details:
                            add_detail_line(f"Subcomponent ({panel_junction_details['Subcomponent']})", None, bullet=True, raw=True)
                except Exception as e:
                    print(f"DEBUG: Exception in junction details display: {e}")
                    pass
    ...
    def process_selected_ehx(evt=None):
        ...
        if pv_mod and hasattr(pv_mod, 'parse_panels'):
            try:
                result = pv_mod.parse_panels(full)
                if result and len(result) >= 3:
                    panels, materials_map, diag_report = result
                elif result and len(result) >= 2:
                    panels, materials_map = result
                    diag_report = None
                else:
                    panels, materials_map, diag_report = [], {}, None
            except Exception:
                panels, materials_map, diag_report = [], {}, None
        else:
            try:
                panels, materials_map, diag_report = parse_panels(full) or ([], {}, None)
            except Exception:
                panels, materials_map, diag_report = [], {}, None
        panels_by_name = {}
        ...
        try:
            ...
            if not writer:
                ...
                writer = write_expected_and_materials_logs
            try:
                writer(full, log_panels, log_materials, diag_report)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(ehx_file):
            print(f"DEBUG: Processing file from command line: {ehx_file}")
            try:
                panels, materials_map, diag_report = parse_panels(ehx_file) or ([], {}, None)
                print(f"DEBUG: Parsed {len(panels)} panels")
                ...
                if not writer:
                    writer = write_expected_and_materials_logs
                writer(ehx_file, panels_by_name, materials_map, diag_report)
                print("DEBUG: Logs written successfully")
                
            except Exception as e:
                print(f"ERROR: Failed to process file: {e}")
        else:
            print(f"ERROR: File not found: {ehx_file}")
    else:
        # GUI mode
