...
def analyze_ehx_files(ehx_path):
    ...
    subassembly_parts = defaultdict(lambda: defaultdict(lambda: {'count': 0, 'fm': '', 'fm_name': '', 'descriptions': []}))
    ...
    if os.path.isfile(ehx_path) and ehx_path.lower().endswith('.ehx'):
        ...
        return subassembly_parts, subassembly_fm, family_member_patterns, subassembly_patterns
                ...
                if guid_el is not None and guid_el.text:
                    ...
                    if guid in subassembly_info:
                        ...
                        if fam_member_name or description:
                            ...
                            subassembly_parts[sub_name][key]['count'] += 1
                            subassembly_parts[sub_name][key]['fm'] = fam_member
                            subassembly_parts[sub_name][key]['fm_name'] = fam_member_name
                            subassembly_parts[sub_name][key]['descriptions'].append(description)
    ...
    if len(ehx_files) == 1:
        ...
        if subassembly_parts:
            parts_count = sum(sum(info['count'] for info in parts.values()) for parts in subassembly_parts.values())
            print(f"Parts in SubAssemblies: {parts_count}")
        
    return subassembly_parts, subassembly_fm, family_member_patterns, subassembly_patterns, beam_pocket_indicators

def generate_pattern_list_file(subassembly_parts, subassembly_fm, family_member_patterns, subassembly_patterns, beam_pocket_indicators, output_file):
    """Generate a comprehensive Pattern List.txt file including SubAssembly parts"""
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for sub_name, fm in subassembly_fm.items():
            if fm in ['32', '42', '25']:
                subassembly_by_fm[fm].append(sub_name)
        ...
        for fm_id in ['32', '42', '25']:
            if fm_id in subassembly_by_fm:
                ...
                for sub_name in sorted(subassembly_by_fm[fm_id]):
                    if sub_name in subassembly_parts:
                        f.write(f"\n• {sub_name} (FM{fm_id})\n")
                        f.write("   Associated Material Parts:\n")
                        for key, info in sorted(subassembly_parts[sub_name].items()):
                            count = info['count']
                            fm = info['fm']
        ...
        for fm_id in loose_fm_ids:
            if fm_id in family_member_patterns:
                for name, count in family_member_patterns[fm_id].items():
                    if name in loose_materials:
                        loose_materials[name] += count
                    else:
                        loose_materials[name] = count
        ...
        for name, count in sorted(loose_materials.items(), key=lambda x: x[1], reverse=True):
        ...
        # List all loose material patterns (excluding SubAssembly FamilyMembers)
        loose_fm_ids = [fm for fm in family_member_patterns.keys() if fm not in ['32', '42', '25']]
        if any(fm in family_member_patterns for fm in loose_fm_ids):
            f.write("\nLOOSE MATERIALS (All FamilyMembers except 32, 42, 25):\n")
            all_loose_patterns = []
            for fm_id in loose_fm_ids:
                if fm_id in family_member_patterns:
                    for name, count in family_member_patterns[fm_id].items():
                        all_loose_patterns.append((fm_id, name, count))
            all_loose_patterns.sort(key=lambda x: x[2], reverse=True)
            for fm_id, name, count in all_loose_patterns:
                f.write(f"- FM{fm_id}: {name} ({count})\n")

        # List all SubAssembly FamilyMember patterns
        subassembly_fm_ids = ['32', '42', '25']
        if any(fm in family_member_patterns for fm in subassembly_fm_ids):
            f.write("\nSUBASSEMBLY FAMILY MEMBERS (32, 42, 25):\n")
            all_sub_patterns = []
            for fm_id in subassembly_fm_ids:
                if fm_id in family_member_patterns:
                    for name, count in family_member_patterns[fm_id].items():
                        all_sub_patterns.append((fm_id, name, count))
            all_sub_patterns.sort(key=lambda x: x[2], reverse=True)
            for fm_id, name, count in all_sub_patterns:
                f.write(f"- FM{fm_id}: {name} ({count})\n")
        ...
        for sub_name, parts in subassembly_parts.items():
            sub_fm = subassembly_fm.get(sub_name, '')
...
def print_analysis(subassembly_parts, subassembly_fm, family_member_patterns, subassembly_patterns, beam_pocket_indicators):
    ...
    print("\nTop Loose Material Patterns (by occurrence):")
    for name, count in sorted(loose_materials.items(), key=lambda x: x[1], reverse=True)[:20]:  # Top 20
        print(f"  {name}: {count}")
...
if __name__ == "__main__":
    ...
    subassembly_parts, subassembly_fm, family_member_patterns, subassembly_patterns, beam_pocket_indicators = analyze_ehx_files(ehx_path)
    print_analysis(subassembly_parts, subassembly_fm, family_member_patterns, subassembly_patterns, beam_pocket_indicators)
    ...
    generate_pattern_list_file(subassembly_parts, subassembly_fm, family_member_patterns, subassembly_patterns, beam_pocket_indicators, output_file)
