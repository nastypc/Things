...
def make_gui():
    ...
    path_val = tk.Label(top, text='(none)', bg=TOP_BG, fg=TEXT_LIGHT, font=('Segoe UI', 10, 'bold'), cursor='hand2')
    ...
    available_levels = []
    selected_level = {'value': None}  # None means all levels
    ...
    def back_clear():
        ...
        try:
            ...
            panels_loaded = False
            selected_panel['name'] = None
            selected_level['value'] = None  # Reset level selection
            available_levels.clear()
            update_level_buttons()  # Clear level buttons
            try:
    ...
    def update_level_buttons():
        ...
        # Collect unique levels from panels (only LevelNo)
        levels = set()
        for p in current_panels.values():
        ...
        # Only show level buttons if there are multiple levels
        if len(available_levels) > 1:
            for level in available_levels:
                btn = tk.Button(level_frame, text=f'L{level}', bg=TOP_BG, fg=TEXT_LIGHT, relief='raised' if selected_level['value'] != level else 'sunken',
                               command=lambda l=level: select_level(l))
                btn.pack(side='left', padx=2)
                level_buttons.append(btn)
        else:
            # If only one level or no levels, don't show buttons and select all
    ...
    def filter_panels_by_level():
        ...
        if selected_level['value'] is None:
            ...
            filtered_panels = dict(current_panels)
            filtered_materials = dict(panel_materials_map)
        else:
            # Filter to selected level
            filtered_panels = {}
            filtered_materials = {}
            for name, panel in current_panels.items():
                panel_level = str(panel.get('LevelNo') or panel.get('Level') or '')
                if panel_level == str(selected_level['value']):
                    filtered_panels[name] = panel
                    if name in panel_materials_map:
                        filtered_materials[name] = panel_materials_map[name]

        # Update file listbox
        file_listbox.delete(0, tk.END)
        for name, panel in filtered_panels.items():
            display_name = panel.get('DisplayLabel', name)
        ...
        # Clear selection and rebuild GUI
        selected_panel['name'] = None
        rebuild_bundles(5)
        # Clear details and breakdown
        for ch in details_scrollable_frame.winfo_children():
            try:
                ch.destroy()
            except Exception:
    ...
    # Level selection buttons (will be populated when EHX is loaded)
    level_frame = tk.Frame(top, bg=TOP_BG)
    level_frame.pack(side='right', padx=6)
    ...
    def rebuild_bundles(count: int):
        ...
        if panels_loaded and current_panels:
            ...
            bundle_panels = {}
            # Prefer using BundleGuid for grouping when available to avoid collisions
            for name, obj in panels_by_name.items():
                ...
                # If no bundle information, use panel name prefix (e.g., "PX" from "PX-001")
                if not bkey and '-' in name:
                    bkey = name.split('-')[0]
                    display_label = bkey

                if not bkey:
                    bkey = 'Bundle'
                # Store a tuple (panel_name, display_label) per group to allow label resolution later
                bundle_panels.setdefault(str(bkey), {'panels': [], 'label': display_label}).get('panels').append(name)

            # Get all bundle keys and paginate them (sort numerically when possible)
            def smart_sort_key(bundle_name):
                # Try to extract number from bundle name (e.g., "Bundle10" -> 10)
                import re
                match = re.search(r'(\d+)', bundle_name)
                if match:
                    return (0, int(match.group(1)), bundle_name)  # Numbers first
                else:
                    return (1, bundle_name, bundle_name)  # Alphabetical fallback
            
            all_bundle_keys = sorted(bundle_panels.keys(), key=smart_sort_key)
            ...
            # Create placeholder buttons in 4x4 grid like buttons.py
            rows = 4
            cols_per_bundle = 4
            # determine available total height for the green zone (fixed) and compute inner bundle height
            total_h = btns_frame.winfo_height() or 150
            per_bundle_h = max(44, int((total_h - 16)))
            ...
            for bi in range(cols):
                ...
                try:
                    bf.grid_propagate(False)
                    bf.configure(height=per_bundle_h)
                except Exception:
                    pass

                # Create 4x4 grid of blank placeholder buttons
                for r in range(rows):
                    for c in range(cols_per_bundle):
                        b = ttk.Button(bf, text='', state='disabled')
                        # Apply reasonable font size to placeholder buttons
                        try:
                            placeholder_font = tkfont.Font(size=10)  # Default size for placeholders
                            b.configure(font=placeholder_font)
                        except Exception:
                            pass
                        try:
                            b.grid(row=r, column=c, sticky='nsew', padx=3, pady=2)
                        except Exception:
                            try:
                                b.pack(side='left', expand=True, fill='both', padx=3, pady=2)
                            except Exception:
                                pass
                        panel_button_widgets.append(b)
    ...
    def process_selected_ehx(evt=None):
        ...
        try:
            # Filter panels and materials by selected level for logging
            if selected_level['value'] is not None:
                ...
                for name, panel in panels_by_name.items():
                    ...
                    if panel_level == str(selected_level['value']):
                        ...
                        if name in panel_materials_map:
                            filtered_materials[name] = panel_materials_map[name]
                log_panels = filtered_panels
                log_materials = filtered_materials
            else:
                # No level selected (single level or all levels), log everything
