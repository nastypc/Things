...
class CDTFile:
    ...
    def generate_glue_lines(self, horizontal_nl_groups: Dict[Tuple[str, float, float], Dict[str, Any]], fmt_value, wall_start: float, wall_end: float, wall_bottom: float, wall_top: float) -> List[str]:
        ...
        # defaults for y/z when templates are missing
        default_y = self.gl_lines[0].y_start if self.gl_lines else (0.5 * (wall_bottom + wall_top))
        default_z = self.gl_lines[0].z_start if self.gl_lines else wall_top
        # clamp default_y away from wall edges when there is room
        try:
            eps = 1e-3
            available_h = float(wall_top) - float(wall_bottom)
            if available_h > 2.0 * float(GLUE_EDGE_OFFSET) + eps:
                min_y = float(wall_bottom) + float(GLUE_EDGE_OFFSET)
                max_y = float(wall_top) - float(GLUE_EDGE_OFFSET)
                if default_y <= float(wall_bottom) + eps:
                    default_y = min_y
                elif default_y >= float(wall_top) - eps:
                    default_y = max_y
                else:
        ...
        # If STA elements exist, prefer per-STA glue emission anchored to STA spans
        try:
            sta_candidates = []
            if getattr(self, 'sta_elements', None):
                ...
                sta_candidates = [s for s in self.sta_elements if is_vertical_support(s)]
                ...
                if horizontal_nl_groups:
                    ...
                    for (y_value, z_value) in rows:
                        for span_start, span_end in sta_spans:
                            gl_start = max(wall_start, span_start + GLUE_EDGE_OFFSET)
                            gl_end = min(wall_end, span_end - GLUE_EDGE_OFFSET)
                            if gl_end <= gl_start:
                            ...
                            out_lines.append(GlueLine._format(fmt_value, gl_start, y_value, z_value, gl_end, y_value, z_value, 0.0, 0.0, tool_index))
                else:
                    for span_start, span_end in sta_spans:
                        gl_start = max(wall_start, span_start + GLUE_EDGE_OFFSET)
                        gl_end = min(wall_end, span_end - GLUE_EDGE_OFFSET)
                        ...
                        if gl_end < gl_start:
                            continue
                        tool_index = default_tool
                        out_lines.append(GlueLine._format(fmt_value, gl_start, default_y, default_z, gl_end, default_y, default_z, 0.0, 0.0, tool_index))
                try:
                    setattr(self, '_last_gl_source', 'xmain_per_sta')
                except Exception:
                    pass
                return out_lines
        except Exception:
            # If STA-based emission fails, continue to normal generation
            pass
        # Determine sensible defaults for Y/Z based on existing GLs or wall extents
        default_y = self.gl_lines[0].y_start if self.gl_lines else (0.5 * (wall_bottom + wall_top))
        default_z = self.gl_lines[0].z_start if self.gl_lines else wall_top
        # Clamp default_y away from absolute wall edges by GLUE_EDGE_OFFSET when room exists
        try:
            epsilon = 1e-3
            available_h = float(wall_top) - float(wall_bottom)
            if available_h > 2.0 * float(GLUE_EDGE_OFFSET) + epsilon:
                min_y = float(wall_bottom) + float(GLUE_EDGE_OFFSET)
                max_y = float(wall_top) - float(GLUE_EDGE_OFFSET)
        ...
        # --- Build STA spans (cluster and dedupe) so we can emit GL per-STA spans ---
        sta_candidates = []
        if getattr(self, 'sta_elements', None):
            def is_vertical_support(candidate) -> bool:
                try:
        ...
        CLUSTER_TOLERANCE = float(getattr(self, 'PRESETS', {}).get('sta_cluster_tolerance', 50.0)) if hasattr(self, 'PRESETS') else 50.0
        sta_spans = []
        if raw_spans:
            cur_start, cur_end = raw_spans[0]
            for s, e in raw_spans[1:]:
        ...
        # If we have STA spans, prefer emitting glue per STA span anchored with inset
        if sta_spans:
            out_lines: List[str] = []
            if horizontal_nl_groups:
                rows = []
                ...
                for (y_value, z_value) in rows:
                    for span_start, span_end in sta_spans:
                        ...
                        if gl_end < gl_start:
                            continue
                        tool_index = default_tool
                        out_lines.append(GlueLine.default_format(fmt_value, gl_start, y_value, z_value, gl_end, y_value, z_value, 0.0, 0.0, tool_index))
                try:
                    setattr(self, '_last_gl_source', 'xmain_per_sta_by_row')
                except Exception:
                    pass
                return out_lines

            # fallback: emit one GL per STA span
            for span_start, span_end in sta_spans:
                gl_start = max(wall_start, span_start + GLUE_EDGE_OFFSET)
                gl_end = min(wall_end, span_end - GLUE_EDGE_OFFSET)
                if gl_end <= gl_start:
                ...
                if gl_end < gl_start:
                    continue
                tool_index = default_tool
                out_lines.append(GlueLine.default_format(fmt_value, gl_start, default_y, default_z, gl_end, default_y, default_z, 0.0, 0.0, tool_index))
            try:
                setattr(self, '_last_gl_source', 'xmain_per_sta')
            except Exception:
                pass
        ...
        for key in ordered_keys:
            ...
            if orient == 'horizontal' and key in horizontal_nl_groups:
                ...
                y_value = info.get('y', template.y_start if template else default_y)
                z_value = info.get('z', template.z_start if template else default_z)
